...
class PathGraphingAstVisitor(ASTVisitor):
    ...
    def visitFunctionDef(self, node):
        ...
        if self.classname:
            entity = '%s%s' % (self.classname, node.username)
        else:
            entity = node.username
        ...
        if self.graph is not None:
            ...
            self.tail = pathnode
            self.dispatch_list(node.body)
            self.graphs["%s%s" % (self.classname, node.username)] = self.graph
            self.reset()
    ...
    if options.dot:
        ...
        for graph in visitor.graphs.values():
            if graph.complexity() >= options.threshold:
                print(graph.username, graph.complexity())
