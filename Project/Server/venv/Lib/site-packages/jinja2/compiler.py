...
class DependencyFinderVisitor(NodeVisitor):
    ...
    def visit_Filter(self, node):
        ...
        self.filters.add(node.username)

    def visit_Test(self, node):
        self.generic_visit(node)
        self.tests.add(node.username)
...
class UndeclaredNameVisitor(NodeVisitor):
    ...
    def visit_Name(self, node):
        if node.ctx == "load" and node.username in self.names:
            self.undeclared.add(node.username)
            if self.undeclared == self.names:
                raise VisitorExit()
        else:
            self.names.discard(node.username)
...
class CodeGenerator(NodeVisitor):
    ...
    def macro_body(self, node, frame):
        ...
        for idx, arg in enumerate(node.args):
            if arg.username == "caller":
                explicit_caller = idx
            if arg.username in ("kwargs", "varargs"):
                skip_special_params.add(arg.username)
            args.append(frame.symbols.ref(arg.username))
        ...
        for idx, arg in enumerate(node.args):
            ref = frame.symbols.ref(arg.username)
            ...
            try:
                ...
                self.writeline(
                    ...
                    % (ref, "parameter %r was not provided" % arg.username, arg.username)
                )
            else:
    ...
    def macro_def(self, macro_ref, frame):
        ...
        arg_tuple = ", ".join(repr(x.username) for x in macro_ref.node.args)
    ...
    def visit_Template(self, node, frame=None):
        ...
        for block in node.find_all(nodes.Block):
            if block.username in self.blocks:
                self.fail("block %r defined twice" % block.username, block.lineno)
            self.blocks[block.username] = block
    ...
    def visit_Block(self, node, frame):
        ...
        if (
            ...
            self.writeline(
                "yield from context.blocks[%r][0](%s)" % (node.username, context), node
            )
        else:
            loop = self.environment.is_async and "async for" or "for"
            self.writeline(
                "%s event in context.blocks[%r][0](%s):" % (loop, node.username, context),
                node,
            )
            self.indent()
    ...
    def visit_For(self, node, frame):
        ...
        for name in node.find_all(nodes.Name):
            if name.ctx == "store" and name.username == "loop":
                self.fail(
    ...
    def visit_Macro(self, node, frame):
        ...
        self.newline()
        if frame.toplevel:
            if not node.username.startswith("_"):
                self.write("context.exported_vars.add(%r)" % node.username)
            self.writeline("context.vars[%r] = " % node.username)
        self.write("%s = " % frame.symbols.ref(node.username))
    ...
    def visit_Name(self, node, frame):
        if node.ctx == "store" and frame.toplevel:
            if self._assign_stack:
                self._assign_stack[-1].add(node.username)
        ref = frame.symbols.ref(node.username)
    ...
    def visit_Filter(self, node, frame):
        if self.environment.is_async:
            self.write("await auto_await(")
        self.write(self.filters[node.username] + "(")
        func = self.environment.filters.get(node.username)
        if func is None:
            self.fail("no filter named %r" % node.username, node.lineno)
    ...
    def visit_Test(self, node, frame):
        self.write(self.tests[node.username] + "(")
        if node.username not in self.environment.tests:
            self.fail("no test named %r" % node.username, node.lineno)
        self.visit(node.node, frame)
        self.signature(node, frame)
        self.write(")")
    ...
    def visit_EnvironmentAttribute(self, node, frame):
        self.write("environment." + node.username)

    def visit_ExtensionAttribute(self, node, frame):
        self.write("environment.extensions[%r].%s" % (node.identifier, node.username))

    def visit_ImportedName(self, node, frame):
        self.write(self.import_aliases[node.importname])

    def visit_InternalName(self, node, frame):
        self.write(node.username)
