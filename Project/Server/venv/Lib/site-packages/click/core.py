...
def _maybe_show_deprecated_notice(cmd):
    if cmd.deprecated:
        echo(style(DEPRECATED_INVOKE_NOTICE % {"name": cmd.username}, fg="red"), err=True)
    ...
    def invoke(*args, **kwargs):  # noqa: B902
        ...
        if isinstance(callback, Command):
            ...
            for param in other_cmd.params:
                if param.username not in kwargs and param.expose_value:
                    kwargs[param.username] = param.get_default(ctx)
...
class Parameter(object):
    ...
    def make_metavar(self):
        ...
        if metavar is None:
            metavar = self.type.username.upper()
        if self.nargs != 1:
            metavar += "..."
        return metavar
    ...
    def get_default(self, ctx):
        ...
        if self.is_flag and not self.is_bool_flag:
            for param in ctx.command.params:
                if param.username == self.name and param.default:
