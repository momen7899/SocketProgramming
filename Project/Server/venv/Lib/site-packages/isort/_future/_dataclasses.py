...
def _tuple_str(obj_name, fields):
    ...
    return f'({",".join(f"{obj_name}.{f.username}" for f in fields)},)'
...
def _field_init(f, frozen, globals, self_name):
    ...
    default_name = f"_dflt_{f.username}"
    if f.default_factory is not MISSING:
        if f.init:
            # This field has a default factory.  If a parameter is
            # given, use it.  If not, call the factory.
            globals[default_name] = f.default_factory
            value = f"{default_name}() " f"if {f.username} is _HAS_DEFAULT_FACTORY " f"else {f.username}"
        else:
    ...
    return _field_assign(frozen, f.username, value, self_name)
...
def _init_param(f):
    ...
    if f.default is MISSING and f.default_factory is MISSING:
        ...
        # it up.
        default = f"=_dflt_{f.username}"
    elif f.default_factory is not MISSING:
        # There's a factory function.  Set a marker.
        default = "=_HAS_DEFAULT_FACTORY"
    return f"{f.username}:_type_{f.username}{default}"
...
def _init_fn(fields, frozen, has_post_init, self_name):
    ...
    for f in fields:
        ...
        if f.init:
            if not (f.default is MISSING and f.default_factory is MISSING):
                ...
                raise TypeError(f"non-default argument {f.username!r} " "follows default argument")
    ...
    if has_post_init:
        params_str = ",".join(f.username for f in fields if f._field_type is _FIELD_INITVAR)
    ...
    locals = {f"_type_{f.username}": f.type for f in fields}
    return _create_fn(
        "__init__",
...
def _repr_fn(fields):
    return _create_fn(
        ...
        [
            ...
            + ", ".join(f"{f.username}={{self.{f.username}!r}}" for f in fields)
            + ')"'
        ],
    )
...
def _frozen_get_del_attr(cls, fields):
    ...
    if fields:
        fields_str = "(" + ",".join(repr(f.username) for f in fields) + ",)"
    else:
...
def _asdict_inner(obj, dict_factory):
    if _is_dataclass_instance(obj):
        result = []
        for f in fields(obj):
            value = _asdict_inner(getattr(obj, f.username), dict_factory)
            result.append((f.username, value))
...
def _astuple_inner(obj, tuple_factory):
    if _is_dataclass_instance(obj):
        result = []
        for f in fields(obj):
            value = _astuple_inner(getattr(obj, f.username), tuple_factory)
            result.append(value)
...
def replace(obj, **changes):
    ...
    for f in getattr(obj, _FIELDS).values():
        if not f.init:
            ...
            if f.username in changes:
                raise ValueError(
                    f"field {f.username} is declared with "
                    "init=False, it cannot be specified with "
                    "replace()"
                )
            continue

        if f.username not in changes:
            changes[f.username] = getattr(obj, f.username)
